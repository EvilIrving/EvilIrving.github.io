...
模块化/组件化: 模块化着重在文件层面上对代码与资源实现拆分与组装，组件化着重在功能层面上对交互与设计实现拆分与组装
...
全局配置 Stylelint,ESlint,Prettier
...
- 缩减范围: 在 loader 中配置include/exclude,适当排除一些不必须要的搜索范围
- 缓存副本: 使用 loader/plugin 有配置缓存 cache 的选项
- 定向搜索: 使用 resolve 提高文件检索速度
- 提前构建: DllPlugin
